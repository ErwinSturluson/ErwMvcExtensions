...
namespace ErwMvcExtensions.ValidationAttributes
{
    ...
    public class NotEqualsToAttribute : ValidationAttribute, IClientValidatable
    {
        private List<string> propertiesToCompareNames;

        public NotEqualsToAttribute(params string[] propertiesToCompareNames)
        {
            this.propertiesToCompareNames = propertiesToCompareNames.ToList();
        }

        public NotEqualsToAttribute(string propertiesToCompareNames)
        ...
        public override string FormatErrorMessage(string name)
        {
            ...
            try
            {
                if (this.ErrorMessageResourceType == null)
                {
                    ...
                    formattingPattern = resourceManager.GetString("NotEqualsTo", CultureInfoExtensions.GetCultureFromHttp(HttpContext.Current.Request));
                }
            ...
            {
                formattingPattern = "{0} must be different from {1}.";
            }
        ...
        public IEnumerable<ModelClientValidationRule> GetClientValidationRules(ModelMetadata metadata, ControllerContext context)
        {
            ...
            foreach (PropertyInfo propertyToCompare in propertiesToCompare)
            {
                ...
                try
                {
                    string propertyDisplayName = resourceManager.GetString(propertyDisplayAttribute.Name);
                    if (string.IsNullOrEmpty(propertyDisplayName))
                    {
                        throw new FormatException("Variable string \"propertyDisplayName\" can't be null or empty.");
                    }
            ...
            yield return new ModelClientValidationNotEqualsToRule(this.FormatErrorMessage(propertiesDisplayNamesString), propertiesToCompareString, propertiesToCompareDisplayNames);
        }
    }

    public class ModelClientValidationNotEqualsToRule : ModelClientValidationRule
    {
        public ModelClientValidationNotEqualsToRule(string errorMessage, string propertiesToCompare, string propertiesToCompareDisplayNames)
        {
